...
public class PortForward {
    ...
    public PortForwardResult forward(String namespace, String name, List<Integer> ports) throws ApiException, IOException {
        ...
        result.setListener(WebSockets.stream(path, "GET", queryParams, apiClient, handler));
    ...
    public static class PortForwardResult { 
        ...
        private WebSockets.Listener listener;
        ...
        public void init() throws IOException {
            for (int i = 0; i < ports.size() * 2; i++) {
                ...
                int r = is.read(data);
                if (r != 2)
                    throw new EOFException("r=" + r);
                int port = Byte.toUnsignedInt(data[0]) + Byte.toUnsignedInt(data[1]) * 256;
                streams.put(port, i / 2);
            }
        }
        ...
        public void setListener(WebSockets.Listener listener) {
            this.listener = listener;
        }

        public WebSockets.Listener getListener() {
            return listener;
        }
    }
}